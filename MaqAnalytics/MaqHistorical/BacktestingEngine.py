...
class BacktestingEngine:
    ...
    def select_model(self):
        ...
        if self.model_type == "logistic_regression":
            ...
            base_model = MLPClassifier(random_state=self.random_state, max_iter=200)
        else:
    ...
    def tune_hyperparameters(self):
        """Hyperparameter tuning & calibration for all supported model types (including neural networks)."""
        ...
        if self.model_type == "logistic_regression":
            ...
            # Simple neural network hyperparameter grid
            param_grid = {
    ...
    def predict_proba_single_game(self, game_features):
        """Predict probability of a home win for a single game."""
        proba = self.pipeline.predict_proba(game_features)[0][1]  # Positive class is home win
        return proba

    def adjust_bookmaker_probabilities(self, home_odds, away_odds):
        """
        Convert home & away moneylines to implied probabilities and normalize
        so they sum to 1 (remove overround).
        """
    ...
    def run_backtest(self, initial_bankroll: float = 10000.0):
        """Run the backtest logic, picking the single best game to bet each day (or no bet)."""
        ...
        backtest_actuals = []
        backtest_profits = []  # track per-bet profit/loss
        ...
        for date, group in tqdm(grouped, total=grouped.ngroups, desc="Processing Dates"):
            # Extract features for this day's games
            ...
            # Find the single best game (largest abs difference from the bookmaker)
            prob_diff = np.abs(prob_home_win - adjusted_prob_home)
            best_game_idx = np.argmax(prob_diff)

            # Extract details for the best game
    ...
    def evaluate_backtest(self, backtest_results: pd.DataFrame, initial_bankroll: float):
        """Evaluate performance metrics, including ROI, Brier Score, Log Loss, etc."""
        ...
        # Statistical tests on daily profits
        profits = np.array(self.backtest_profits)
        profits = profits[profits != 0]  # optionally remove 0-profit bets
        ...
        # Diebold-Mariano test on difference in Brier Score
        loss_diff = np.array([
            (mp - ao) ** 2 - (bp - ao) ** 2
    ...
    def run_full_pipeline(self, initial_bankroll: float = 10000.0):
        """Wraps backtest, evaluation, and reporting steps."""
    ...
    def get_feature_importances(self):
        """
        Attempt to extract feature importances from the calibrated classifier.
        For SVM, neural_network, etc., it won't be available, so we skip that part gracefully.
        """
        ...
        if self.model_type in ["random_forest", "gradient_boosting", "xgboost"]:
            for calibrated_clf in calibrated_classifier.calibrated_classifiers_:
                ...
                if hasattr(estimator, 'feature_importances_'):
                    ...
                    print("Estimator does not have feature_importances_.")
                    return None
    ...
    def generate_report(self, results):
        """Generate a text report summarizing all backtest metrics & final results."""
