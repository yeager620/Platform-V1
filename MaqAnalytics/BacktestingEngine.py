    ...
    def run_backtest(self, initial_bankroll: float = 10000.0):
        ...
        for date, group in tqdm(grouped, total=grouped.ngroups, desc="Processing Dates"):
            ...
            for index, game in group.iterrows():
                ...
                actual_outcome = game[self.target_column]  # 1 = home win, 0 = away win

                # Extract feature vector for the game
                feature_columns = [col for col in self.backtest_data.columns if
                                   col not in self.moneyline_columns + [self.target_column, 'Game_Date', 'Game_PK']]
                ...
                # Collect probabilities and actual outcomes for all games
                all_model_probs.append(prob_home_win)
                ...
                all_bookmaker_probs.append(adjusted_prob_home)
            ...
            for game_prob in game_probs:
                X_new = game_prob['game_features']
                y_new = pd.Series([game_prob['actual_outcome']])
